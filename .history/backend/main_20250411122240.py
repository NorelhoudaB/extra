...
def combine_files(file_one, file_two, file_three):
    ...
    styles = '\n'.join([style_dict[Path(path).stem] for path in input_files])
    merged_body_raw = ''.join([body_dict[Path(path).stem] for path in input_files])
    merged_body = sanitize_html_attributes(merged_body_raw)
    ...
    # Sanitize and normalize: remove duplicate <html>, <body> etc. from merged content
    clean_body_soup = BeautifulSoup(merged_body, 'lxml')
    extracted_body_parts = []
    for el in clean_body_soup.body.contents if clean_body_soup.body else clean_body_soup.contents:
        if isinstance(el, str) or el.name not in ['html', 'body']:
            extracted_body_parts.append(el)
    ...
    if soup_two.body:
        soup_two.body.clear()
        for el in extracted_body_parts:
            soup_two.body.append(el)
