...
async def optimise_images(
    ...
    try:
        ...
        if not file: raise HTTPException(status_code=500, detail=f"Erreur : Veuillez choisir les fichier HTML")
        tmpdir = "tmp"
        ...
        logging.error(
            f"Erreur lors de l'optimisation du fichier : {file.filename} - {e}"
        )
