...
async def fix_alt(file: UploadFile = File(...)):
    ...
    return JSONResponse(content={"processed_content": updated_content, "download_url": download_url})


@app.get("/download/{filename}")
async def download_file(filename: str):
    processed_file_path = f"{UPLOAD_DIR}/processed_{filename}"
    print(f"Serving file: {processed_file_path}")
    return FileResponse(processed_file_path, filename=f"processed_{filename}", media_type="text/html")