...
def rename_batch_classes_in_file(input_html):
    pattern = r'(?<=class=["\'])([^"\']+)(?=["\'])'
    ...
    updated_html = re.sub(pattern, replace_classes, input_html)
...
def rename_a_classes_in_file(input_html):
    pattern = r'(?<=class=["\'])([^"\']+)(?=["\'])'
...
async def merge_file(file1: UploadFile = File(...), file2: UploadFile = File(...), file3: UploadFile = File(...)):
    ...
    # Merge styles and body contents
    merged_styles = '\n'.join(style_dict.values())
    merged_body = ''.join(body_dict.values())

    # Create the merged HTML content
    merged_html = f"<html><head><style>{merged_styles}</style></head><body>{merged_body}</body></html>"

    # Apply class renaming (batch -> REPLACED and A -> CHANGED)
    merged_html = rename_batch_classes_in_file(merged_html)
    merged_html = rename_a_classes_in_file(merged_html)

    # Save the merged HTML to a file
    merged_output_file = "merged_output.html"
    with open(merged_output_file, 'w', encoding='utf-8') as f:
        f.write(merged_html)

    # Return the merged HTML file as a downloadable response using FileResponse
    return FileResponse(merged_output_file, media_type="text/html", headers={"Content-Disposition": f"attachment; filename={merged_output_file}"})
