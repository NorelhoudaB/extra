...
async def optimise_images(file: UploadFile = File(...), del_jpeg: int = 0):
    try:
        print(f"🖼️ Starting image optimization: {file.filename}")
        if not file:
        ...
        if not file_content:
            print("⚠️ The file is empty or corrupted")
...
async def fix_space(file: UploadFile = File(...)):
    ...
    file_path = os.path.join(UPLOAD_DIR, file.filename)
    processed_filename = file.filename.replace(".xhtml", "_processed.xhtml").replace(".html", "_processed.html")
    processed_path = os.path.join(UPLOAD_DIR, processed_filename)
