...
async def convert_xhtml(file: UploadFile = File(...)):
    ...
    html_path = os.path.join(UPLOAD_DIR, file.filename.rsplit(".", 1)[0] + ".html")
    ...
    def xhtml_to_html(xhtml_file, html_file):
        ...
        if tree.docinfo.doctype:
            tree.docinfo.doctype = None
        tree.getroot().attrib.pop("xmlns", None)

        html_content = etree.tostring(tree.getroot(), method="html", encoding="unicode")
