...
PNGQUANT_URL = "https://github.com/kornelski/pngquant/releases/latest/download/pngquant-windows.zip"
INSTALL_DIR = os.path.join(os.getcwd(), "pngquant_bin")
PNGQUANT_PATH = os.path.join(INSTALL_DIR, "pngquant.exe")

def ensure_pngquant():
    if not os.path.exists(PNGQUANT_PATH):
        print("🔍 pngquant not found. Downloading...")
        os.makedirs(INSTALL_DIR, exist_ok=True)

        zip_path = os.path.join(INSTALL_DIR, "pngquant.zip")
        urllib.request.urlretrieve(PNGQUANT_URL, zip_path)

        with zipfile.ZipFile(zip_path, "r") as zip_ref:
            zip_ref.extractall(INSTALL_DIR)

        # Move the executable to the correct place
        extracted_dir = [d for d in os.listdir(INSTALL_DIR) if os.path.isdir(os.path.join(INSTALL_DIR, d))][0]
        shutil.move(os.path.join(INSTALL_DIR, extracted_dir, "pngquant.exe"), PNGQUANT_PATH)
        shutil.rmtree(os.path.join(INSTALL_DIR, extracted_dir))
        os.remove(zip_path)
        print(f"✅ pngquant installed at {PNGQUANT_PATH}")
