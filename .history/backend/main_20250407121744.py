...
def rename_batch_classes_in_file(input_file):
...
def rename_a_classes_in_file(input_file):
    with open(input_file, 'r', encoding='utf-8') as file:
        html = file.read()
...
async def merge_file(file1: UploadFile = File(...), file2: UploadFile = File(...), file3: UploadFile = File(...)):
    ...
    # Return the merged HTML file as a downloadable response
    with open(merged_output_file, 'rb') as f:
        return FileResponse(f.read(), media_type="text/html", headers={"Content-Disposition": f"attachment; filename={merged_output_file}"})