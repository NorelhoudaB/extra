...
<template>
  <div class="sidebar d-flex flex-column align-items-center" :style="{ width: isCollapsed ? '60px' : '250px' }"   @mouseover="isCollapsed = false" @mouseleave="isCollapsed = true">
    ...
    <div class="logo-container">
      <img v-if="!isCollapsed" src="/amelkislogo.png" class="logo" :class="{ active: route.path === '/' }" @click.prevent="selectOption('/')" />
      <img v-else src="/amelkislogomenu.png" class="logo" />
    </div>
    ...
    <ul class="nav flex-column w-100 mt-4">
      <li class="nav-item">
        <a href="/" class="nav-link" :class="{ active: route.path === '/' }" @click.prevent="selectOption('/')">
          <span class="nav-text" v-show="!isCollapsed">Réduire la taille du fichier</span> <i class="bi bi-house-fill"></i>
        </a>
      </li>
      <li class="nav-item">
